...
| Fix | Why it works |
| --- | --- |
| The container animates its own height across the swap | The page below settles instead of snapping. It is motion's layout animation, so the height change is a transform with a scale correction on the children rather than a real height tween — off the main thread. |
| The skeleton leaves faster than the content arrives — 120ms out against 260ms in | Never a frame with both at full strength, never one with neither. A skeleton lingering over the real text is the one thing worse than a hard swap. |
...
resize — which is the kind of detail nobody can name but everybody sees.
