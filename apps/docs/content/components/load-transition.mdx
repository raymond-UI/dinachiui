...
<ComponentPreview name="load-transition-default" title="Default" description="Watch the panel resize rather than snap" />

<ComponentPreview name="load-transition-thresholds" title="Thresholds" description="A fast load never shows a skeleton at all" />
...
260ms in. There is never a frame with both at full strength and never one with neither. A
skeleton lingering over the real text is the one thing worse than a hard swap.
