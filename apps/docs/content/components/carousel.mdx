...
both match what the hand meant. The projection is measured from where the strip *is*, not
from how far the finger travelled.
...
- **Each slide is a group** with `aria-roledescription="slide"` and a name that includes its
  position — `2 of 4: Velocity handoff`. Position is the part a screen reader user cannot
  get any other way.
